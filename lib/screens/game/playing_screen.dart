...
import '../../widgets/game_clear_dialog.dart';
...
// posisi bubble dekat kids
const double kBubbleOffsetX = 54;
const double kBubbleOverlapOnKids = 22;
...
enum QuestionKind { number, letter, mcq }
...
class McqQuestion {
  ...
  McqQuestion(this.prompt, this.options, this.answer, {this.imagePath})
    : assert(options.length >= 2 && options.contains(answer));
}
...
class _Item {
  ...
  const _Item.number(this.number)
    : kind = QuestionKind.number,
      letter = null,
      mcq = null;
  const _Item.letter(this.letter)
    : kind = QuestionKind.letter,
      number = null,
      mcq = null;
  const _Item.mcq(this.mcq)
...
class _PlayingScreenState extends State<PlayingScreen> {
  ...
  void initState() {
    super.initState();
    final all = _buildItems(widget);
    final limit = min(
      10,
      all.length,
    ); // mainkan 10 soal (atau kurang kalau pool lebih sedikit)
    _items = all.take(limit).toList(); // batasi list
    _targetCount = limit; // simpan target
  }
  ...
  List<_Item> _buildItems(PlayingScreen w) {
    switch (w.grade) {
      ...
      case GameGrade.grade2:
        final img = _grade2AdjectiveImagePool();
        final noImg = _grade2AdjectiveNoImagePool();
        return <_Item>[
          for (final q in img) _Item.mcq(q),
          for (final q in noImg) _Item.mcq(q),
        ];
      case GameGrade.grade3:
        final q3 = _grade3DemoPool();
        return [for (final q in q3) _Item.mcq(q)];
    }
  }

  double get _progress {
    if (_targetCount == 0) return 0.0;
    return (_index / _targetCount).clamp(0.0, 1.0);
  }
  ...
  Future<void> _showEndDialog(bool win) async {
    ...
    if (win) {
      await showGameClearDialog(
        context: context,
        onBack: () {
          // tutup dialog lalu kembali ke screen sebelumnya
    ...
    // fallback sederhana untuk kondisi kalah
    await showDialog(
      context: context,
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Kesempatan salah habis.'),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.of(context).pop();
              ...
              children: [
                ...
                Expanded(
                  child: switch (item.kind) {
                    ...
                    QuestionKind.mcq => _GradeMcqView(
                      q: item.mcq!,
                      bubbleText: "Can you help me?",
                      onPick: (v) => _onAnswer(v == item.mcq!.answer),
                    ),
                  },
                ),
              ],
  ...
  /// ---------------- Grade 2: Adjective (image) ----------------
  List<McqQuestion> _grade2AdjectiveImagePool() => [
    McqQuestion(
      "Kemala's father is . . . than Kemala",
      ["taller", "shorter", "smaller"],
      "taller",
      imagePath: 'asset/images/learning_grade2_adjective.jpg',
    ),
    McqQuestion(
  ...
  /// ---------------- Grade 2: Adjective (non-image) ----------------
  List<McqQuestion> _grade2AdjectiveNoImagePool() => [
    McqQuestion("This puzzle is . . . than the last one", [
      "easier",
      "easiest",
      "easy",
    ], "easier"),
    McqQuestion("My bag is . . . than yours", [
      "heavier",
      "lightest",
      "heavy",
    ], "heavier"),
    McqQuestion("Today is . . . than yesterday", [
      "colder",
      "coldest",
      "cold",
    ], "colder"),
    McqQuestion("The red car is . . . than the blue car", [
      "faster",
      "fastest",
      "fast",
    ], "faster"),
    McqQuestion("A snail is . . . than a rabbit", [
      "slower",
      "slowest",
      "slow",
    ], "slower"),
  ...
  /// ---------------- Grade 3: demo MCQ pool (ganti sesuai kebutuhan) ----------------
  List<McqQuestion> _grade3DemoPool() => [
    McqQuestion(
      "Choose the correct past tense: 'She _____ to school yesterday.'",
      ["goes", "went", "gone"],
      "went",
    ),
    McqQuestion("Which is the synonym of 'big'?", [
      "large",
      "tiny",
      "short",
    ], "large"),
    McqQuestion("Which sentence is correct?", [
      "He don't like apples",
      "He doesn't like apples",
      "He not like apples",
    ], "He doesn't like apples"),
    McqQuestion("Which is an adjective?", [
      "quickly",
      "quick",
      "quickerly",
    ], "quick"),
    McqQuestion("Find the antonym of 'cold'", [
      "freezing",
      "chilly",
      "hot",
    ], "hot"),
    McqQuestion("Complete: 'The cat is ____ than the dog.'", [
      "smaller",
      "smallest",
      "small",
    ], "smaller"),
    ...
    McqQuestion("Which word is a noun?", ["run", "beauty", "quick"], "beauty"),
    McqQuestion("Pick the correct article: '___ apple a day.'", [
      "A",
      "An",
      "The",
    ], "An"),
    McqQuestion("Which sentence uses future tense?", [
      "I eat now",
      "I ate",
      "I will eat",
    ], "I will eat"),
...
/// =======================
/// GRADE 1: Number (bubble + word, opsi di kanan acak)
/// =======================
...
/// =======================
/// GRADE 1: Letter (board kanan + bubble dekat kids)
/// =======================
              ...
              Positioned.fill(
                child: Column(
                  children: [
                    ...
                    // opsi huruf: lebar mengikuti papan, center di bawah papan
                    Align(
                      alignment: Alignment.centerRight,
                      child: ConstrainedBox(
                        constraints: BoxConstraints(maxWidth: boardMaxWidth),
...
/// =======================
/// GRADE 2/3 MCQ view (layout sama seperti letter)
/// =======================
class _GradeMcqView extends StatelessWidget {
  const _GradeMcqView({
    required this.q,
    required this.onPick,
    this.bubbleText = "Choose the correct answer",
  });
          ...
          return Stack(
            children: [
              ...
              Positioned(
                left: kBubbleOffsetX,
                bottom: kKidsHeight - kBubbleOverlapOnKids,
                child: _SpeechBubble(text: bubbleText, maxWidth: 200),
              ),
            ],
          );
        },
